...
static int
bpred_default_ob_width(unsigned int l2size)
{
  return l2size ? (int)l2size : 16384;
}
	     ...
	     unsigned int frmt)	/* FRMTs Enabled Flag */
{
  ...
  case BPredOB:
	...
	// Outcome Buffer (OB)
	bpred_ob_create(pred, bpred_default_ob_width(l2size)); /* scale OB width with predictor table size */
  ...
  case BPredMBP:
	...
	case BPredLLBP:
		...
		/* FALLTHROUGH: LLBP reuses the TSCL initialization path below. */
	case BPredTSCL:	
		// TAGE-SC-L
		...
		struct stat_sdb_t *sdb)	/* stats database */
{
  ...
  sprintf(buf, "%s.ob_lookups", name);
  stat_reg_counter(sdb, buf, "number of Outcome Buffer probes", &pred->ob_lookups, 0, NULL);
  sprintf(buf, "%s.ob_hits", name);
  stat_reg_counter(sdb, buf, "number of Outcome Buffer hits", &pred->ob_hits, 0, NULL);
  sprintf(buf, "%s.fhb_lookups", name);
  stat_reg_counter(sdb, buf, "number of FHB probes", &pred->fhb_lookups, 0, NULL);
  sprintf(buf, "%s.fhb_hits", name);
  stat_reg_counter(sdb, buf, "number of FHB hits", &pred->fhb_hits, 0, NULL);
  sprintf(buf, "%s.oht_lookups", name);
  stat_reg_counter(sdb, buf, "number of OHT probes", &pred->oht_lookups, 0, NULL);
  sprintf(buf, "%s.oht_hits", name);
  stat_reg_counter(sdb, buf, "number of OHT hits", &pred->oht_hits, 0, NULL);
  sprintf(buf, "%s.frmt_lookups", name);
  stat_reg_counter(sdb, buf, "number of FRMT lookups", &pred->frmt_lookups, 0, NULL);
  sprintf(buf, "%s.frmt_hits", name);
  stat_reg_counter(sdb, buf, "number of FRMT hits", &pred->frmt_hits, 0, NULL);
  sprintf(buf, "%s.frmt_misses", name);
  stat_reg_counter(sdb, buf, "number of FRMT misses", &pred->frmt_misses, 0, NULL);
  sprintf(buf, "%s.tage_lookups", name);
  stat_reg_counter(sdb, buf, "number of TAGE lookup attempts", &pred->tage_lookups, 0, NULL);
  sprintf(buf, "%s.tage_tag_matches", name);
  stat_reg_counter(sdb, buf, "number of TAGE tag matches", &pred->tage_tag_matches, 0, NULL);
  sprintf(buf, "%s.tage_sc_lookups", name);
  stat_reg_counter(sdb, buf, "number of SC summation evaluations", &pred->tage_sc_lookups, 0, NULL);
  sprintf(buf, "%s.tage_sc_inversions", name);
  stat_reg_counter(sdb, buf, "number of SC-driven inversions", &pred->tage_sc_inversions, 0, NULL);
  sprintf(buf, "%s.loop_lookups", name);
  stat_reg_counter(sdb, buf, "number of loop predictor lookups", &pred->loop_lookups, 0, NULL);
  sprintf(buf, "%s.loop_hits", name);
  stat_reg_counter(sdb, buf, "number of loop predictor hits", &pred->loop_hits, 0, NULL);
  sprintf(buf, "%s.llbp_lookups", name);
  stat_reg_counter(sdb, buf, "number of LLBP lookup attempts", &pred->llbp_lookups, 0, NULL);
  sprintf(buf, "%s.llbp_hits", name);
  stat_reg_counter(sdb, buf, "number of LLBP hits", &pred->llbp_hits, 0, NULL);
}
...
bpred_after_priming(struct bpred_t *bpred)
{
  ...
  bpred->oht_lookups = 0;
  bpred->oht_hits = 0;
  bpred->frmt_lookups = 0;
  bpred->frmt_hits = 0;
  bpred->frmt_misses = 0;
  bpred->tage_lookups = 0;
  bpred->tage_tag_matches = 0;
  bpred->tage_sc_lookups = 0;
  bpred->tage_sc_inversions = 0;
  bpred->loop_lookups = 0;
  bpred->loop_hits = 0;
  bpred->llbp_lookups = 0;
  bpred->llbp_hits = 0;
}
...
int unmasked_key_from_frmt(
	...
	if (mapped) pred->frmt_hits++; else pred->frmt_misses++;
	return mapped;
}
...
int context_from_frmt(
	...
	mapped = pred->fwd_dirpred.frmt->context[cont];
	if (mapped) pred->frmt_hits++; else pred->frmt_misses++;
	return mapped;
}
		 ...
		 int frmt)  				/* FRMTs enabled flag */
{
	...
	switch (pred->class) {
		...
		case BPred2bit:
			if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND)) {
				/* Probe OB/FHB/OHT in priority order. Use an explicit validity flag so a valid not-taken
				 * outcome (0) is not mistaken for the absence of a value. */
				if ((pred->class==BPredMBP)||(pred->class==BPredTSCL)||(pred->class==BPredLLBP)||(pred->class==BPredOB)) {
					pred->ob_lookups++;
					if ((!flow_mode) && pred->ob.fv[pred->ob.end]) {
						pred->ob_hits++;
						fwd_valid_outcome = pred->ob.oc[pred->ob.end];
						have_fwd_valid_outcome = TRUE;
					}
				}

				if (!have_fwd_valid_outcome && ((pred->class==BPredMBP)||(pred->class==BPredTSCL)||(pred->class==BPredLLBP)||
		...
		switch (pred->class) {
			...
			case BPred2bit:
				if ((MD_OP_FLAGS(op) & (F_CTRL|F_UNCOND)) != (F_CTRL|F_UNCOND)) {
					...
					if (!have_rev_valid_outcome && ((pred->class==BPredMBP)||(pred->class==BPredTSCL)||(pred->class==BPredLLBP)||
					    (pred->class==BPredOB)||(pred->class==BPredOHT)||(pred->class==BPred2Level))) {
						pred->fhb_lookups++;
